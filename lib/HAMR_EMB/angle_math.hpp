...
public:
  ...
  static Rev32 FromRad(float val) {
    ...
    const float pre_coef = 1.0f/(2.0f*kPi);
    val = pre_coef * val;
    val = val - static_cast<int>(val);
    if(val < 0) {
      val = val + 1;
    }
    const float post_coef = pow2<float>(32);
    val = val * post_coef;
  ...
  float ToRad2Pi() {
    const float coef = kPi / (pow2<std::uint64_t>(31));
...
public:
  ...
  static RevI15Q16 FromRad(float val) {
    ...
    const float coef = pow2<float>(16)/(2.0f*kPi);
  ...
  float ToRad() {
    const float coef = kPi / (pow2<std::uint32_t>(15)); // 2*pi/2^16
    return static_cast<float>(value_) * coef;
  };
